...
class Profesor : public Usuario{
	...
	public:
		Profesor(std::string correo, std::string carrera, std::string nombre, std::string materia, int valoracion) : Usuario(correo, carrera, nombre), m(materia), v(valoracion), c(carrera){
			if (v < 1) {
				v = 1;
			}
		}
